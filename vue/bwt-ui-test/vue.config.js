...
module.exports = {
  ...
  // 例如 https://www.bwton.com/。如果应用被部署在一个子路径上，你就需要用这个选项指定这个子路径。例如，如果你的应用被部署在 https://www.bwton.com/admin/，则设置 baseUrl 为 /admin/。
  ...
  devServer: {
    ...
    proxy: {
      "/api": {
        target: `https://dev.bwton.cn/bwextra-dev01/bwe-data`,
        changeOrigin: true,
        pathRewrite: {
          "^/api": "",
        },
  ...
  configureWebpack: {
    ...
    output: {
      library: `${name}`,
      libraryTarget: "umd", //微前端 把子应用打包成 umd 库格式
